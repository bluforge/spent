...
function Tile({ label, value, sub }: { label: string; value: string; sub?: string }) {
  return (
    <div className="card p-4">
      ...
      <div className="mt-1 text-[21px] font-bold leading-tight">{value}</div>
      {sub && (
      ...
      {empty ? (
        <section className="card mt-4 flex flex-col items-center px-5 py-12 text-center">
          ...
          <p className="mt-1 text-xs" style={{ color: 'var(--ink-3)' }}>
            Dodaj unos pomoću zelenog + dugmeta
          </p>
        </section>
      ) : (
        <>
          <section className="card mt-4 p-5">
            <h2 className="text-[15px] font-semibold">Mesečna potrošnja</h2>
            <div className="mt-4">
          ...
          {topCats.length > 0 && (
            <section className="card mt-3 p-5">
              <h2 className="text-[15px] font-semibold">Top kategorije (6 mes.)</h2>
